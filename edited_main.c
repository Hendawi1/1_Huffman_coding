...
void encode(char **huff, FILE *input, FILE *encoded){
	rewind(input);
       	char c;
       	int len = 0;
...
void decode(FILE *encoded, FILE *decoded){
	...
	// export len and num_of_el from encoded file:
		int len, num_of_el;
		fscanf(encoded, "%d %d\n", &num_of_el, &len);
		printf("len: %d\n", len);
	

	// export encoded text from encoded file:
		char c;
		fread(&c,1,1,encoded);
		size_t bytes = calc_bytes_from_bits(len);
		unsigned char *encoded_as_bin = (unsigned char*)calloc(bytes, sizeof(char));
		for (int i=0; c != '\n'; i++) {encoded_as_bin[i] = c; fread(&c,1,1,encoded);}


	// export freq from encoded file:
		int freq[256] = {0}, val;		
		fscanf(encoded, "%c%d ", &c, &val);
	...
	// use exportet freq to build the tree:
...
int main(){
	...
	printf("\n\n[encoded]:\n");
	for (int i=0; i < 256; i++) if (freq[i]) num_of_el++;
	fprintf(encoded, "%d ", num_of_el);
	encode(huff, input, encoded);
	fprintf(encoded, "\n");
	for (int i=0; i < 256; i++) 
		if (freq[i]){
			fprintf(encoded, "%c", i);
			fprintf(encoded, "%d ", freq[i]);
		}
	//fprintf(encoded, "\n");
	fclose(encoded);
